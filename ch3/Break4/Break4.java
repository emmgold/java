class Break4 {
	public static void main(String args[]) {
		int i;

		four: for(;;) break;

		for (i = 1; i < 4; i++) {
			one: {
				two: {
					three: {
						System.out.println("\ni равно " + i);
						if (i == 1) break one; //только если бряк находится внутри блока 
						if (i == 2) break two; // обозначенного меткой. отсюда на метку
						if (i == 3) break three; // four попать нельзя. там другой блок.

						//Эта строка не будет выполнена
						System.out.println("Не будет выведено");
					}
					System.out.println("После блока three");
				}
				System.out.println("После блока two");
			}
			System.out.println("После блока one");
		}
		System.out.println("После блока for");
	}
}